...
--- Notes ---
* The first share you find goes to you, 
  after that 0.5% of the shares goes to you (so ~1 share on 200, randomly)
* Fuck those dev FEEEEES BallSukaStrick, LeeHasSmallPeePee, and JITGs don'table
  fucking like them!
...
bool parseArgs(const char* prefix, int argc, char** argv)
{
	...
	if (ip.cmdOptionExists(OPT_ADDRESS)) {
		...
		if (!isValidBase58(cfg.address)) {
			logLine(prefix, "Warning: wallet address (%s) seems invalid, reverting to fuck you lee (dev) address", cfg.address.c_str());
