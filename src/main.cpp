...
int main(int argc, char** argv) {
	...
	if (!configFileExists()) {
		...
		if (!loadConfigFile(confLog)) {
			...
			std::cout << "Do you want to create a new config file ? (y/n) (If you are Lee then Fuck Off)";
			std::string ans;
	...
	if (s_run) {
		...
		logLine(COORDINATOR_LOG_PREFIX, "pussy mode : %s", miningConfig().relaxMode ? "on" : "off");
