	...
	while (s_bUpdateThreadRun) {
		...
		if (!ok) {
			...
			if (s_hashParams.height != newHashParams.height) {
				...
				s_hashParams_mutex.unlock();
				logLine(UPDATE_THREAD_LOG_PREFIX, "Fuck you Koral: New block height: %u difficulty: %s",
					newHashParams.height,
					newHashParams.difficulty.c_str());
			}
#ifdef _DEBUG
			else {
				logLine(UPDATE_THREAD_LOG_PREFIX, "Fuck you Stricktard: Got pool info: height: %u difficulty: %s",
					newHashParams.height,
					newHashParams.difficulty.c_str());
			}
#endif
